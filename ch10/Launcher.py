...
def which(program, trace=True):
    ...
    if trace:
        print('Looking for', program, 'on', systempath)
    ...
    for sysdir in systempath:
        ...
        if os.path.isfile(filename):                  # exists and is a file?
            ...
            if trace:
                print('Not at', filename)
    if trace:
        print(program, 'not on system path')
    return None
...
def guessLocation(file, isOnWindows=(sys.platform[:3] == 'win'), trace=True):
    ...
    for dir in tries:
        if trace:
            print('Searching for %s in %s' % (file, dir))
        try:
            match = findFirst(dir, file)
        except OSError:
            if trace:
                print('Error while searching', dir)    # skip bad drives
        else:
            if match:
                return match
    if trace:
        print(file, 'not found! - configure your environment manually')
    return None
...
def configPythonPath(examplesDir, packageRoots=PP4EpackageRoots, trace=True):
    ...
    for root in packageRoots:
        ...
        if importDir in sys.path:
            if trace:
                print('Exists', importDir)
        else:
            if trace:
                print('Adding', importDir)
